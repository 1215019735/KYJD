...
<body>
        ...
        <div class="core">
            ...
            <ul class="public-nav">
                <?php if(is_array($nav)): $i = 0; $__LIST__ = $nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$nav): $mod = ($i % 2 );++$i;?><li>
                        <a href="<?php echo U('Server/index',array('id'=>$nav['id']));?>"><?php echo ($nav["name"]); ?></a>
                    </li><?php endforeach; endif; else: echo "" ;endif; ?>
            </ul>
            <div class="server-box">
                <div class="server-page-main">
                    <?php if(is_array($data["list"])): $i = 0; $__LIST__ = $data["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Server/content',array('id'=>$vo['id']));?>">
                            <img src="<?php echo ($vo["img"]); ?>" width="100%" />
                            <div class="server-page-title"><?php echo ($vo["name"]); ?></div>
    ...
    <script>
        var navLength = $('.public-nav>li').length;
        var href = window.location.href.toLowerCase();
        for (let i = 0; i < navLength; i++) {
            var url = $('.public-nav>li').eq(i).children('a').attr('href').toLowerCase();
            if (href.lastIndexOf(url) > -1) {
                $('.public-nav>li').eq(i).addClass('hover').siblings().removeClass('hover');
            }
        }
    </script>
